...
import { _ROUTES, _SIZE, _SUBSCRIPTION_MONTHS, _COUNTRIES } from "../../constants/GlobalSetting";
...
class SubscriptionInformation extends Component {
	...
	componentDidUpdate(prevProps) {
		...
		if (prevProps.set_shipping !== set_shipping) {
			...
			if (success) {
				this.props.history.push(_ROUTES.SUBSCRIPTION_PAYMENT)
			}
		}
	}
	...
	render() {
		const { products, todaysTotal, monthlyTotal, formValues: { address, cityState, country, email, phone }, formErrors } = this.state;
...
export default connect(mapStateToProps, mapDispatchToProps)(SubscriptionInformation);